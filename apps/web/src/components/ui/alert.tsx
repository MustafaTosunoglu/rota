  ...
  'relative w-full rounded-lg border px-4 py-3 text-sm grid grid-cols-[0_1fr] has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current',
  {
    variants: {
      variant: {
        ...
        destructive: 'text-destructive [&>svg]:text-current border-destructive/30 bg-destructive/5',
        success: 'text-green-700 dark:text-green-400 border-green-600/30 bg-green-600/5',
      },
    },
    defaultVariants: {
      variant: 'default',
